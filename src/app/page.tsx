...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setDebugLog("MiniKit SDK Initialized via Official NPM Package.");
    } catch (e) {
  ...
  const handleRunAgent = async () => {
    ...
    try {
      ...
      // 🚨 CRITICAL BYPASS: 1 USDC simulation fallback
      setProposal({ 
      ...
      setDebugLog("Generated safe approve(1000000) simulation payload.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExecute = async () => {
    ...
    setErrorMsg(null);
    setTxHash(null);
    setDebugLog("Preparing payload via official MiniKit NPM Package...");
    ...
    try {
      // THE DEFINITIVE BRIDGE (Using official commandsAsync to talk to the hardware)
      // We append a safe .catch() inline so Vercel doesn't hang if the wallet swallows the promise
      MiniKit.commandsAsync.sendTransaction({
        transaction: proposal.txData,
        reference: `wldguard-tx-${Date.now()}`
      }).catch((e) => console.log("Internal Promise caught:", e));
      
      setDebugLog("Payload successfully handed to native app wallet...");

      // Safety Net Timeout
      setTimeout(() => {
        setIsExecuting((currentlyExecuting) => {
          if (currentlyExecuting) {
            setErrorMsg("Hardware Timeout: Ensure World App is updated and the Paymaster accepts this route.");
